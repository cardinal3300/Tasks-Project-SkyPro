...
### 1. Модуль `main.py` содержит функцию:
...
*   **Возвращаемое значение:**
    ...
    *   list[list] | list[dict] | list[Any] | None: Список словарей с разными данными или ни чего.
...
*   **Пример:**
    ...
    >>> выводит разный подсчет транзакций, то что прописывал в программе пользователь 

### 2. Модуль `masks.py` содержит функции:
...
### 3. Модуль `widget.py` содержит функции:
...
### 4. Модуль `processing.py` содержит функции:
...
### 5. Модуль `generators.py` содержит функции:
...
### 6. Модуль `decorators.py` содержит функцию и декоратор:
...
### 7. Модуль `utils.py` содержит функцию:
...
### 8. Модуль `external_api.py` содержит функции:

### `convert_currency(currency: str, amount: float)`

Пример использования:

Функция конвертирует валюту в рубли, предполагая наличие API-ключа и валидных данных.
...
### 9. Модуль `csv_excel.py` содержит функции:
