...
def test_mask_account_card_parametrize(input_string: str, expected_output: str) -> str:
    assert mask_account_card(input_string) == expected_output


def test_mask_account_card_account(test_data: str) -> str:
    """Проверяет маскирование счета."""
    assert mask_account_card("Счет 123456789012") == "Счет **9012"


def test_mask_account_card_card(test_data: str) -> str:
    """Проверяет маскирование карты."""
    assert mask_account_card("Visa 1234567890123456") == "Visa 1234 56** **** 3456"


def test_mask_account_card_invalid(test_data: str) -> str:
...
def test_get_date_valid(test_data: Any) -> str:
    """Проверяет корректное преобразование даты."""
    assert get_date("2024-03-11T02:26:18.671407") == "11.03.2024"


def test_get_date_str(test_data: int) -> str:
